...
import { message, Table, Switch, Button, Modal, Space,Rate  } from 'antd';
import { Link } from "react-router-dom";


function ReatingPage() {
    ...
    const MenuColumn = [
     ...
     
        {
            ...
            render: (text, record) => (
                <span><Link to="/Admin/rating-view">
                <i className="fa fa-eye p-2  rounded-circle " style={{ color: 'white', backgroundColor: 'black' }} id={record.id} ></i>
                </Link>
                   <i className="fa fa-trash p-2  rounded-circle mx-2" style={{ color: 'white', backgroundColor: 'black' }} id={record.id} onClick={MenuDelete}></i></span>
            )
        },
    ]
